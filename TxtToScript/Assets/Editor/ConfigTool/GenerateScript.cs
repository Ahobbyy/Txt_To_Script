...
public class GenerateScript
{
    ...
    [MenuItem("Tools/Config/GenerateScript")]
    public static void GenerateScriptTool()
    {
        Init();
    ...
    /// 生成脚本
    /// </summary>
    private static void ToScript(string dataName, string data)
    {
    ...
    private static void CreateDatabaseScript(TextAsset textAsset)
    {
        ...
        ToScript(textAsset.name + "Database", template);
    }
    ...
    private static string GetCsvSerialize(TextAsset textAsset)
    {
        ...
        for (int i = 0; i < keyCount; i++)
        {
            ...
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToArray<string>(m_datas[i][{1}]);", attributes[1], i);
            }
            else if (attributes[0] == "bool[]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToArray<bool>(m_datas[i][{1}]);", attributes[1], i);
            }
            else if (attributes[0] == "int[]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToArray<int>(m_datas[i][{1}]);", attributes[1], i);
            }
            else if (attributes[0] == "float[]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToArray<float>(m_datas[i][{1}]);", attributes[1], i);
            }
    ...
    private static void CreateDatabaseManagerScript()
    {
        ...
        ToScript("ConfigDataManager", template);
    }
}
